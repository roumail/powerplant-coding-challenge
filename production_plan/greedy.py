...
def calculate_production_plan(payload: Payload, method: str = "dummy") -> list[ResponsePowerPlant]:
    if method == 'dummy':
...
def allocate_power(load: int, fuels: dict[str, float], powerplants: list[dict[str, float]]) -> list[dict[str, float]]:
